...
forma_wdpa = "SELECT COUNT(f.*) AS value FROM forma_api f, (SELECT * FROM protected_areas WHERE wdpaid=1) AS p WHERE ST_Intersects(f.the_geom, p.the_geom) AND f.date >= '2014-01-01'::date AND f.date <= '2015-01-01'::date"

forma_use = "SELECT COUNT(f.*) AS value FROM logging_gcs_wgs84 u, forma_api f WHERE u.cartodb_id = 1 AND ST_Intersects(f.the_geom, u.the_geom) AND f.date >= '2014-01-01'::date AND f.date <= '2015-01-01'::date"
